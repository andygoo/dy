...
<div class="page-container" style=" padding-left:50px;">
	<form action="" method="post" class="form form-horizontal" id="form-product-add" enctype="multipart/form-data" onkeydown="if(event.keyCode==13)return false;">
		<div class="row cl">
			<input type="hidden" value="<?php echo ($planinfo["p_id"]); ?>" name="p_id">
			<label class="form-label col-xs-4 col-sm-1" style=" width:115px;"><span class="c-red">*</span>计划名称：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="<?php echo ($planinfo["p_name"]); ?>" placeholder="" id="name" name="p_name" style=" width:200px;">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-1" style=" width:115px;"><span class="c-red">*</span>每日预算：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="<?php echo ($planinfo["p_repnum"]); ?>" placeholder="每日预算不能少于100" id="maxRmb" name="p_repnum" style=" width: 200px;">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-1" style=" width:115px;"><span class="c-red">*</span>广告出价：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" value="<?php echo ($planinfo["p_price"]); ?>" readonly placeholder="" id="price" name="p_price" style=" width: 200px;" />
			</div>
		</div>
		<div class="row cl">
...
<script type="text/javascript">
    ...
    function save_submit() {
        if($('#name').val().length<=0){
            ...
            $.post("<?php echo U('Editplan/index');?>",
                {
                    data:getdata
                },function (e) {
                    e=JSON.parse(e);
